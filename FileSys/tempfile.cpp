...
#include "FileSys.h"
  ...
  void FileSys::mount(int sock) {
    ...
    curr_dir =
        1; // by default current directory is home directory, in disk block #1
    fs_sock = sock; // use this socket to receive file system operations from
                    // the client and send back response messages
  }
  ...
  void FileSys::mkdir(const char *name) {
    ...
    int set = 0;
    do {
      if (root.dir_entries[i].block_num ==
          0) { // Fixed: assignment to comparison
        root.dir_entries[i].block_num =
            block.dir_entries[0]
                .block_num; // Fixed: incorrect reference to block_num
        set = 1;
  ...
  // compares two separate char* names assumes name1 and name2 are null
  // terminated returns True if they match, false otherwise.
