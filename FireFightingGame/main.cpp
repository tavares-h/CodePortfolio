...
struct Map {
  ...
  std::vector<std::pair<int, int>>
      fire_location;  // Added to store fire coordinates
  int fire_total = 0; // Added to track total fires
  Map() : map_state(mapRows, std::vector<char>(mapCols, ' ')) {}
};

// made this so new levels can get a fresh map
...
// this just updates the screen with the map
...
// this will be the implementation for spot fire generation
int SpotFireChance(int fires, int chance) { return fires % chance; }

// this randomly generates whether a fire happens
// if the fire does happen it randomly propagates
...
int main() {
  ...
  // made a prev and cur just for flexibility
  Map map_cur;
  Map map_prev;

  MapInit(map_cur); // initialize the map
